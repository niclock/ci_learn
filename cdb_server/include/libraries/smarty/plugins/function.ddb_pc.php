...
function smarty_function_ddb_pc($params, &$smarty)
{
    ...
    if (!empty($aset['fieldname'])){
        ...
        $result = db_query("SELECT {$aset['fieldname']} FROM ".table($aset['tabledb'])." ".$wheresql.$orderbysql.$limit);
    }else{
        $result = db_query("SELECT * FROM ".table($aset['tabledb'])." ".$wheresql.$orderbysql.$limit);
    }
    $list= array();
    $__n= 1;$_timearrid = '';
