...
function tpl_form_aledit($name = '', $value = '', $options = array()) {
    ...
    if (!defined('TPL_INIT_ALEDIT')) {
        ...
        <script type="text/javascript">
        	window.TPL_INIT_AL_USERID = 0;
        </script>';
        define('TPL_INIT_ALEDIT', true);
    }
    if (empty($name)) $name = "__tpl_form_aledit";
    $showtab = 'text,sms,imagetext,image,video,voice,template';
    $opt = array();
    $opt['name'] = $name;
    ...
    $sc.= '<script type="text/javascript">
        $(function(){ $("#'.$opt['id'].'").aliedit({seltab:"", callback:"", home: "'.$_A['url']['index'].'", BASE_URI: "'.BASE_URI.'", IMG_PATH: "'.IMG_PATH.'", alid: "0", value: '.json_encode($value).', showtab: "'.$showtab.'"}); });
    </script>';
    if ($name == "__tpl_form_aledit") {
        $s = '<div style="display:none">'.$s.'</div>';
        $sc.= '<script type="text/javascript">
        $.fn.tpl_form_aledit = function(seltab, callback) {
            this.aliedit({seltab:seltab||"", callback:callback||"", home: "'.$_A['url']['index'].'", BASE_URI: "'.BASE_URI.'", IMG_PATH: "'.IMG_PATH.'", alid: "0", value: '.json_encode($value).', showtab: "'.$showtab.'"});
        };
        </script>';
...
function tpl_form_imagemore($name, $value = '', $max = 10, $default = '', $options = array(), $callback = null, $hidepreview = false) {
	...
	$s = '';
    $s.= __tpl_load_cssjs(JS_PATH.'formfile/global.css', true);
		...
		<script type="text/javascript">
			...
			function showImageDialogmore(elm, opts, options, callback) {
                ...
                imgbox.find("input").each(function(i){
					if ($(this).val()) {
						val[$(this).val()] = $(this).next().attr("data-src");
					}
				});
			    $.formfilemore(function(data){
                     ...
                     $.each(data.images, function(n,v) {
							...
							j++;
							imgbox.append(\'<div class="inimem"><dl><dd><input type="hidden" name="\'+btn.attr("data-name")+\'[]" value="\'+n+\'" autocomplete="off"><a href="\' + v + \'" target="_blank" data-src="\' + v + \'" style="background-image:url(\' + v + \')"></a><em class="close" title="删除这张图片" onclick="deleteImageDialogmore(this, \\\'' . $callback . '\\\');">×</em></dd></dl></div>\');
						}
					});
					if (callback) {
						eval(callback);
			...
			function deleteImageDialogmore(elm, callback){
				if ($(elm).parents(".formfile-inputbox").find("input").length > 1) {
                	    $(elm).parents(".inimem").remove();
				}else{
					$(elm).prev().attr("data-src", $default);
					$(elm).prev().css("background-image", "url(' . $default . ')");
	...
	$valuearr = $value;
	if (!is_array($value)) {
	    $tmparr = string2array($value);
		$valuearr = $tmparr?$tmparr:array($value);
	}
	$valbox = '';
	foreach($valuearr AS $item) {
		if ($item) {
			$valbox.= '<div class="inimem"><dl><dd><input type="hidden" name="'.$name.'[]" value="'.$item.'" autocomplete="off"><a href="'.fillurl($item).'" target="_blank" data-src="'.fillurl($item).'"  style="background-image:url('.fillurl($item).')"></a><em class="close" title="删除这张图片" onclick="deleteImageDialogmore(this, \''.$callback.'\');">×</em></dd></dl></div>';
		}
	}
	if (empty($valbox)) {
		$valbox.= '<div class="inimem"><dl><dd><a href="'.$default.'" target="_blank" data-src="'.$default.'" style="background-image:url('.$default.')"></a><em class="close" title="删除这张图片" onclick="deleteImageDialogmore(this, \''.$callback.'\');">×</em></dd></dl></div>';
	}
...
function tpl_form_image($name, $value = '', $default = '', $options = array(), $callback = null, $hidepreview = false) {
    ...
    if (!defined('TPL_INIT_IMAGE')) {
		...
		<script type="text/javascript">
			...
			function onmouseImageDialog(elm, type) {
			    var ipt = $(elm);
			    if (ipt.attr("data-hidesbox") != "1") {
			        return;
			    }
			    var box = ipt.parents(".formfile-inputbox");
			    var sbox = box.next(".formfile-inputbox");
			    if (type) {
			        sbox.css({
			            "top": box.offset().top - $(window).scrollTop() + box.height(),
			...
			function onmouseBoxDialog(elm, type) {
			    var sbox = $(elm);
			    if (sbox.attr("data-hidesbox") != "1") {
			        return;
			    }
			    if (type) {
			        sbox.show();
...
<div class="input-group formfile-inputbox '. $options['class_extra'] .'">
	<input type="text" id="'.$name.'" name="'.$name.'" value="'.$value.'"'.($options['extras']['text'] ? $options['extras']['text'] : '').' class="form-control" data-hidesbox="'.$hidepreview.'" onmouseover="onmouseImageDialog(this, true)" onmouseout="onmouseImageDialog(this, false)" onblur="onblurImageDialog(this)" autocomplete="off"><span class="input-group-btn"><button class="btn-default" type="button" onclick="showImageDialog(this, \'' . base64_encode(serialize($options)) . '\', '. str_replace('"','\'', json_encode($options)).', \''.$callback.'\');">选择图片</button></span>
</div>';
    if(!empty($options['tabs']['browser']) || !empty($options['tabs']['upload'])){
		$hidepreviewsty = '';
		if ($hidepreview) {
		...
		$s .=
			'<div class="input-group formfile-inputbox '. $options['class_extra'] .'" style="margin-top:.5em;'.$hidepreviewsty.'" data-hidesbox="'.$hidepreview.'" onmouseover="onmouseBoxDialog(this, true)" onmouseout="onmouseBoxDialog(this, false)">
