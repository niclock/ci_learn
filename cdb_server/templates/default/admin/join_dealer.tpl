...
<div id="dcWrap">
    ...
    {#tpl('_top')#}
    <!-- dcHead 结束 -->
    {#tpl('_left', 'join_dealer')#}

    <div id="dcMain" class="smstmpl">
        <!-- 当前位置 -->
        <div id="urHere">管理中心<b>&gt;</b><strong>商家入驻</strong></div>
                            ...
                            <form action="{#url(0)#}" method="get" class="filtertable" id="filtertable">
                                <table>
                                    ...
                                    <tr>
                                        <td>
                                            <div>门店名称</div>
                                            <input name="storename" type="text" class="inpMain" value="{#$_GPC['storename']#}"/>
                                        </td>
                                        <td>
                                            <div>门店地址</div>
                                            <input name="address" type="text" class="inpMain" value="{#$_GPC['address']#}"/>
                                        </td>
                                        <td class="last">
                                            <div class="filterlr">
                                                <div class="filterl">
                                    ...
                                    <tr>
                                        <td colspan="3" class="fill">
                                            <div class="fastselectstatus">
                                                快速搜索:
                                                <a class="btnGray status" href="{#url(0)#}?status={#urlencode('审核中')#}">审核中</a>
                                                <a class="btnGray status" href="{#url(0)#}?status={#urlencode('已审核')#}">已审核</a>
                                                {#if $_where != $wheresql#}
                        ...
                        <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                            ...
                            {#foreach from=$lists['list'] item=item#}
                                <tr>
                                    ...
                                    <td>{#$item['payid']#}</td>
                                    <td>{#$item['paytype']#}{#if $item['paysubtype']#}-{#$item['paysubtype']#}{#/if#}</td>
                                    <td>{#$item['username']#}<a class="tdsub" href="{#url('admin/users')#}?userid={#$item['userid']#}" target="_blank">ID: {#$item['userid']#}</a> </td>
                                    <td class="normal">{#$item['storename']#}</td>
                                    <td class="normal">{#$item['address']#}</td>
                                    <td>{#$item['userphone']#}</td>
                                    <td align="center">&yen;{#$item['money']#}</td>
                                    <td align="center">{#date("Y-m-d H:i", $item['indate'])#}</td>
                                    <td align="center">{#$item['status']#}</td>
                                    <td align="center" title="{#if $item['update']#}{#date("Y-m-d H:i", $item['update'])#}{#/if#}"><a href="javascript:;" onclick="dosubmit('{#$item['id']#}');">标记已审</a> | <a href="javascript:;" onclick="dosubmit('{#$item['id']#}', true);">标记未审</a></td>
                                </tr>
                                {#foreachelse#}
                                <tr>
...
<script>
    ...
    function update(status, id) {
        var url = '{#get_link('act|status|ctx|id')#}';
        url+= '&act=update';
        url+= '&status=' + encodeURIComponent(status);
        url+= '&id=' + id;
        url+= '&ctx=' + encodeURIComponent(window.location.href);
        window.location.href = url
    }
    function dosubmit(id, no) {
        var title = '确认提示',
            content = '是否确定标记为已审核？',
            buttons = [{
                name: '已审核',
