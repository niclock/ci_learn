$.__baidu_map_fun = {
    ...
    show: function(paramet, callback){
        ...
        var intemp = $('<div class="jQuery-baidumap">' +
            ...
            '<div class="baidumap-search-resultpanel" style="display:none;"></div>' +
            '<em class="lock-title unlock" id="__baidumap_lock" onclick="$.__baidu_map_fun.locktitle(this);"></em>' +
        ...
        //
        if (paramet.title != '') {
            $("#__baidumap_lock").click();
        }
    },
    close: function() {
        $("div.jQuery-baidumap").remove();
    },
    locktitle: function(obj){
        $("#__baidumap-title").toggleClass("__lock");
    ...
    baidu_map: function(data, showobjid, addrobjid, lngobjid, longobjid, butgobjid, hiderobjid) {
        ...
        if (data && data.lng && data.lat) {
            ...
            if ($("#"+lngobjid).val() && $("#"+longobjid).val()){
                var point = new BMap.Point($("#"+lngobjid).val(), $("#"+longobjid).val());
            }else{
                var point = new BMap.Point(108.397933,22.835299);
            }
            doit(point);
        ...
        function doit(point) {
            if (point) {
                ...
                $('#'+lngobjid).val(point.lng);
                $('#'+longobjid).val(point.lat);
                ...
                myGeo.getLocation(point, function (result) {
                    if (result) {
                        if (!hiderobjid && result.address && $('#'+addrobjid).val() == "") {
                            $('#'+addrobjid).val(result.address);
                        }
                    }
                });
                ...
                marker2.addEventListener("dragend", function (e) {
                    $('#'+lngobjid).val(e.point.lng);
                    $('#'+longobjid).val(e.point.lat);
                    myGeo.getLocation(new BMap.Point(e.point.lng, e.point.lat), function (result) {
                        if (result) {
                            if (!hiderobjid && result.address) {
                                if (!$('#'+addrobjid).hasClass("__lock")) {
                                    $('#'+addrobjid).val(result.address);
                                }
                            }
