 ...
 * @param but           赋值显示按钮名称
 * @param bk            是否隐藏背景
 * @param callback      隐藏后执行的函数
 */
jQuery.alert = function(e, t, p, but, bk, callback) {
    ...
    if (but) e+= but;
    var n = '<div class="jQuery-ui-alert" style="position:fixed;top:0;left:0;padding:15px 10px;min-width:100px;opacity:1;min-height:25px;text-align:center;color:#333;z-index:2147483647;border-radius:3px;background-color: #ffffff; opacity:1;font-size:14px;display:table;max-width:76%;" id="jQuery-ui-alert-' + m + '" ><div style="display:table-cell;vertical-align:middle;">' + e + '</div></div>' +
    ...
    if((ipad || isMobile) && $(window).width()>=640) {
        nobj.css({"padding":"25px 20px","font-size":"20px", "line-height":"28px"});
    }else{
        nobj.css({"padding":"15px 10px","font-size":"14px", "line-height":"22px"});
    }
    if (!p)	nobj.click(function(){
        nobj.removeClass("jQuery-ui-alert-style").fadeOut('normal', function(){
            if (typeof callback == 'function') { callback(); }
        });
        nobjbg.hide();
    });
    if (!p)	nobjbg.click(function(){
        nobj.removeClass("jQuery-ui-alert-style").fadeOut('normal', function(){
            if (typeof callback == 'function') { callback(); }
        });
        nobjbg.hide();
    ...
    if (window.jQuery_alert_resize !== true) {
        window.jQuery_alert_resize = true;
        $(window).resize(function(){
            $("div.jQuery-ui-alert").each(function(){
                var obj = $(this);
                var i = $(window).width(),
                    s = $(window).height(),
                    o = obj.outerWidth(),
                    ...
                    l = (i - o) / 2,
                    top = (s - u) / 2 - 20;
                i > o && obj.css("left", parseInt(l));
                i > o && obj.css("right", parseInt(l));
                s > u && obj.css("top", top);
                obj.removeClass('jQuery-ui-alert-style');
            });
        });
    }
...
jQuery.alertk = function(e, t, p, but, callback) {
    $.alert(e, t, p, but, 1, callback)
};
...
jQuery.confirm = function(e, qfun, cfun, q, c) {
    ...
    var ipad = ua.match(/(iPad).*OS\s([\d_]+)/),
        ...
        isMobile = isIphone || isAndroid,
        div_margin = '15px -10px -15px',
        div_padding = '10px 18px';
    if((ipad || isMobile) && $(window).width()>=640) {
        div_margin = '20px -20px -25px';
        div_padding = '16px 24px';
    }
    //
    if (typeof e == "object") {
        ...
        var _tempbut = '<div style="margin:'+div_margin+';border-top:1px solid #dddddd;display:-webkit-box;">';
        ...
        $.each(e.button, function (i, val) {
            var attcss = '';
            if (i > 0) { attcss = 'border-left:1px solid #dddddd;margin-left:-1px'; }
