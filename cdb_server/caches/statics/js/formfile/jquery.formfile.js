$.__formfile_fun = {
    ...
    inimages: function(callback, imgurl, indexurl, isanimate){
        ...
        var intemp = $('<div class="jQuery-formfile">' +
            ...
            '<input type="file" name="formfile_images" class="formfile_images" accept="image/*">' +
            '</p>' +
            '</div>' +
            '<div class="formfile-button"><p class="hover" id="f-confirm">确定</p><p id="f-cancel">取消</p></div> ' +
            '</div>' +
            '</div>');
        if (imgurl && tthis.varp.inimagesismore !== true) {
        ...
        intemp.find(".formfile_imagebut").click(function(){
            if (tthis.varp.inimagesismore === true) {
                if (tthis.varp.inimagesismoremax > 0 && tthis.arrlength(tthis.varp.imgurl) >= tthis.varp.inimagesismoremax) {
                    tthis.call("最多只能选：" + tthis.varp.inimagesismoremax + "张", 0, 1);
                    return false;
                }
            }
        ...
        intemp.find("#f-confirm").click(function(){
            if (!intemp.attr("data-subtitle") || intemp.attr("data-subtitle") == 0) {
                ...
                if (intemp.find(".formfile_readonly").val()) {
                    ...
                    var ajaxurl = fileurl + 'admin/upfile/' + fileid + '/images/?1=1';
                    ...
                    $.ajaxFileUpload({
                        ...
                        data: {
                            thumb_width: typeof window.__upfile_thumb_width === 'number'?window.__upfile_thumb_width:0,
                            thumb_height: typeof window.__upfile_thumb_height === 'number'?window.__upfile_thumb_height:0
                        },
                        dataType: 'json',
                        success: function (data, status) {
                            if (data != null && data.success != null && data.success) {
                                ...
                                if (callback) {
                                    if (tthis.varp.inimagesismore === true) {
                                        tthis.varp.moreimg = tthis.varp.imgurl;
                                        tthis.varp.moreimg[data.upload_data.full_path_site] = data.upload_data.full_path_new;
                                        callback({'images':tthis.varp.moreimg, 'num':tthis.arrlength(tthis.varp.moreimg)});
                                    }else{
    ...
    viewimages: function(callback, indexurl, nowval, imgurl){
        ...
        //点击确定
        intemp.find("#f-confirm").click(function(){
            if (tthis.varp.inimagesismore === true && tthis.arrlength(tthis.varp.moreimg) > 0) {
                if (callback) callback({'images':tthis.varp.moreimg, 'num':tthis.arrlength(tthis.varp.moreimg)});
                tthis.close();
            }else{
                tthis.call("请点击列表选择");
            }
                ...
                if ($(obj).attr("data-f")){
                    ...
                    if (tthis.varp.inimagesismore === true) {
                        if ($(obj).hasClass("fselect")) {
                            ...
                            tthis.varp.moreimg[$(obj).find("#fileurl").val()] = "";
                        }else{
                            if (tthis.varp.inimagesismoremax > 0 && tthis.arrlength(tthis.varp.moreimg) >= tthis.varp.inimagesismoremax) {
                                tthis.call("最多只能选：" + tthis.varp.inimagesismoremax + "张", 0, 1);
                            }else{
                                $(obj).addClass("fselect");
                                tthis.varp.moreimg[$(obj).find("#fileurl").val()] = fullpath;
                            }
                        }
                        tthis.alert("已选：" + tthis.arrlength(tthis.varp.moreimg) + "张");
                        return true;
                    }else{
                ...
                success: function (html) {
                    ...
                    if (tthis.varp.inimagesismore === true) {
                        eve.find(".formfile-fileview").find("li").each(function () {
                            var fileval = tthis.varp.moreimg[$(this).find("input#fileurl").val()];
        ...
        intemp.find("#f-confirm").click(function(){
            if (!intemp.attr("data-subtitle") || intemp.attr("data-subtitle") == 0) {
                ...
                if (intemp.find(".formfile_readonly").val()) {
                    var fileid = intemp.find(".formfile_images").attr("id");
                    var fileurl = indexurl?indexurl:$("body").attr("data-index");
                    var ajaxurl = fileurl + 'admin/upfile/' + fileid + '/' + uptype + '/?allowed=' + allowed + '&size=' + size;
                    tthis.call("正在上传", 1);
                    if (window.TPL_INIT_AL_USERID) { ajaxurl+= "&userid="+window.TPL_INIT_AL_USERID }
                    $.ajaxFileUpload({
                        url: ajaxurl,
    ...
    del: function(obj){
        if (confirm("确定删除并且不可恢复吗？")) {
            ...
            $.ajax({
                type: "GET",
                url: fileurl + "admin/delup?path=" + eve.find("#fileurl").val(),
                dataType: "html",
                success: function (html) {
    ...
    arrlength: function(arr) {
        var j = 0;
        if (this.varp.inimagesismore === true) {
...
$.formfile = function(callback, imgurl, indexurl, uptype, allowed, size) {
    ...
    tthis.varp.inimagesismore = false;
...
$.formfilemore = function(callback, imgurl, indexurl, max) {
    ...
    tthis.varp.inimagesismore = true;
    tthis.varp.inimagesismoremax = max;
    tthis.varp.imgurl = imgurl;
    tthis.loadjscssfile(tthis.path + "/default.css", "css", function(){
        tthis.inimages(callback, imgurl, indexurl);
    });
};